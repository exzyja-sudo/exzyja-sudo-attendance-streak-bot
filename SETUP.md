...
channel, and choose an `access-role` that should see the poll. Enter exactly two options
separated by `|`, for example `No | Yes`, and a duration such as `30s`, `5m`, `1h`, or
`1d` (maximum `7d`). The bot adds 🟢 to the first option and 🔴 to the second, counts
